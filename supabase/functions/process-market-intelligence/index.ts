...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!record || !record.metadata) {
      return new Response(JSON.stringify({ success: true, message: 'No metadata' }), { headers: corsHeaders })
    }
    ...
    if (record.metadata.type === 'oferta') {
      ...
      if (!condoId && condoName) {
        const { data: existingCondo } = await supabase.from('condominiums')
          .select('id')
        ...
        if (existingCondo) {
          ...
          const { data: newCondo } = await supabase.from('condominiums')
      ...
      const propBairro = (record.metadata.bairro || record.metadata.neighborhood || '').toLowerCase();
      const propTipo = (record.metadata.tipo_imovel || '').toLowerCase();
      const propValor = Number(record.metadata.valor || 0);
      
      const { data: demands } = await supabase.from('documents')
        .select('id, metadata')
      ...
      if (demands) {
        for (const demand of demands) {
          const demandBairro = (demand.metadata.bairro || demand.metadata.region || '').toLowerCase();
          const demandTipo = (demand.metadata.tipo_imovel || '').toLowerCase();
          const demandValor = Number(demand.metadata.valor || 0);
          
          let matchScore = 0;
          if (demandBairro && propBairro && (propBairro.includes(demandBairro) || demandBairro.includes(propBairro))) {
            matchScore += 0.4;
          }
          if (demandTipo && propTipo && (propTipo.includes(demandTipo) || demandTipo.includes(propTipo))) {
            matchScore += 0.3;
          }
      ...
      for (const condoName of demandedCondos) {
        ...
        if (condoId) {
          await supabase.from('demanda_condominio').insert({
            demand_id: record.id,
            condominium_id: condoId,
