    ...
    const processWebhook = async (payload: any) => {
      ...
      try {
        ...
        for (const demand of demands) {
          ...
          const isValuePartial = propValor > demandValor && propValor <= demandValor * 1.10
          ...
          const isLegacyMatch = !property.metadata.bairro && propLocation.includes(demandEndereco) && (isValuePerfect || isValuePartial)
          
          const isMatch = (isTypeMatch && (isValuePerfect || isValuePartial) && isLocationMatch) || isLegacyMatch
          ...
          if (isMatch && demand.metadata.user_id) {
            ...
            if (brokerProfile && brokerProfile.whatsapp_number) {
              ...
              notificationPromises.push(
                ...
                }).then(({ error }) => { if (error) console.error('Error triggering WhatsApp invoke:', error) })
              )
            }
          }
        }
