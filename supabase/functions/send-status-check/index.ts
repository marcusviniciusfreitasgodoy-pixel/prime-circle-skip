      ...
      for (const { me, partner } of brokers) {
        ...
        if (!me.whatsapp_number) {
          ...
          await supabase.rpc('log_notification', {
            p_user_id: me.id, p_recipient: 'unknown', p_channel: 'whatsapp', p_status: 'failed',
        ...
        if (isReminder) {
          const { data: existingTokens } = await supabase.from('quick_action_tokens')
        ...
        if (tokens.length === 0) {
          ...
          const { data: newTokens, error: tokensError } = await supabase.from('quick_action_tokens').insert([
        ...
        let text = ''
        const prefix = isReminder ? `⏳ *Lembrete PrimeCircle*\n\nOi ${firstNameMe}, ainda não tivemos retorno sobre o ` : `Oi ${firstNameMe} 👋 *PrimeCircle* aqui.\n\n`

        if (currentStatus === 'match') {
          text = `${prefix}match com *${firstNamePartner}* (${propertyType} · ${propertyRegion}) confirmado há dias.\n\nO contato já aconteceu? Toque no link:\n\n🔗 Sim, já falamos\n${url1}\n\n🔗 Ainda não consegui\n${url2}\n\n🔗 Match não faz mais sentido\n${url3}\n\n_Cada link funciona uma única vez._`
