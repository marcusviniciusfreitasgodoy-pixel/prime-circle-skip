  ...
  try {
    ...
    for (const p of partnerships) {
      ...
      const sendToBroker = async (broker: any, partner: any) => {
        ...
        const { data: tokenData } = await supabase.from('quick_action_tokens').insert({
          partnership_id: p.id,
        ...
        const displayStatusMap: Record<string, string> = { 'contact': 'CONTATO', 'visit': 'VISITA', 'proposal': 'PROPOSTA', 'closed': 'FECHAMENTO' }
