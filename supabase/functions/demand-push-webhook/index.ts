  ...
  try {
    ...
    const processWebhook = async (payload: any) => {
      ...
      try {
        ...
        const demandRegion = record.metadata.region || record.metadata.bairro || record.metadata.endereco || 'sua região'
        ...
        const { data: subs, error } = await supabase.from('user_push_subscriptions').select('user_id')
        ...
        if (activeProfiles && activeProfiles.length > 0) {
          for (const profile of activeProfiles) {
            ...
            if (profile.whatsapp_number) {
              ...
              let waMessage = `Olá ${partnerName}! 🚀 Uma nova demanda foi cadastrada na Prime Circle: ${demandDetails}. Acesse a plataforma para conferir e oferecer seus imóveis: https://prime-circle-migration-fd549.goskip.app/dashboard`
