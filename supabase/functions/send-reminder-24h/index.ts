  ...
  try {
    ...
    for (const p of partnerships) {
      ...
      const propertyRegion =
        (p.property as any)?.metadata?.bairro || (p.property as any)?.metadata?.region || 'Região'
      ...
      for (const { me, partner } of brokers) {
        ...
        const firstNamePartner = (partner as any).full_name
          ? (partner as any).full_name.split(' ')[0]
          : 'Parceiro'
