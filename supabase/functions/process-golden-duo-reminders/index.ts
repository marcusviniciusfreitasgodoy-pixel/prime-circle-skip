...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!profiles || profiles.length === 0) {
      return new Response(JSON.stringify({ success: true, message: 'No profiles to process' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json'} })
    }
    ...
    for (const profile of profiles) {
      ...
      if (offers > 0 && demands === 0) {
        ...
        await supabase.from('profiles').update({ golden_duo_reminder_sent_at: new Date().toISOString() }).eq('id', profile.id)
        continue
      } else {
        continue
      }
