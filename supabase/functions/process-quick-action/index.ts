...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!token || !status_alvo || !corretor_id) {
      return new Response(JSON.stringify({ valido: false, motivo: 'invalido' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    if (tokenRecord.used) {
      return new Response(JSON.stringify({ valido: false, motivo: 'ja_usado' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    
    if (expiresAt < now) {
      return new Response(JSON.stringify({ valido: false, motivo: 'expirado' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
    ...
    const [{ data: partnerProfile }, { data: propertyDoc }, { data: corretorProfile }] = await Promise.all([
    ...
    if (action === 1) { // Avançar
      if (status_alvo !== 'aguardando_vgv') {
        await supabase.from('partnerships').update({
          ...
          last_updated_by: corretor_id // 5. Para o trigger do banco capturar
        }).eq('id', partnership.id)

        pontos_ganhos = 2
        novo_status = status_alvo

        // 4. Validação da notificação ao parceiro se o telefone faltar
        if (partnerProfile?.whatsapp_number) {
          ...
          console.warn(`Parceiro ${partnerId} sem telefone. Notificação de avanço falhou silenciosamente.`);
          await supabase.rpc('log_notification', {
            p_user_id: partnerId, p_recipient: 'unknown', p_channel: 'whatsapp', p_status: 'failed',
            p_message_body: `Broker avançou negociação para ${status_alvo}`,
            p_error_details: 'Telefone ausente'
          });
        }
      } else { // status_alvo == 'aguardando_vgv'
        await supabase.from('partnerships').update({
          status: 'aguardando_vgv',
          vgv_confirmado_a: vgv,
          updated_at: new Date().toISOString(),
          last_interaction_at: new Date().toISOString(),
          last_updated_by: corretor_id // 5. Para o trigger do banco
        }).eq('id', partnership.id)
        ...
        if (partnerProfile?.whatsapp_number) {
          const formattedVgv = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(vgv || 0)
          ...
          console.warn(`Parceiro ${partnerId} sem telefone. Notificação de VGV falhou silenciosamente.`);
          await supabase.rpc('log_notification', {
            p_user_id: partnerId, p_recipient: 'unknown', p_channel: 'whatsapp', p_status: 'failed',
            p_message_body: `Informado fechamento de imóvel (VGV: ${vgv})`,
            p_error_details: 'Telefone ausente'
          });
        }
      }
    } else if (action === 2) { // Manter
      await supabase.from('partnerships').update({
        last_status_check_at: new Date().toISOString(),
        last_updated_by: corretor_id
      }).eq('id', partnership.id)
    } else if (action === 3) { // Encerrar
      await supabase.from('partnerships').update({
        status: 'cancelado',
      ...
      if (partnerProfile?.whatsapp_number) {
        ...
        await supabase.rpc('log_notification', {
            p_user_id: partnerId, p_recipient: 'unknown', p_channel: 'whatsapp', p_status: 'failed',
            p_message_body: `Match encerrado pelo parceiro`,
            p_error_details: 'Telefone ausente'
          });
      }
    }
    ...
    return new Response(JSON.stringify({ valido: false, motivo: 'erro_interno', error: error.message }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  }
})
