  ...
  try {
    ...
    if (action === 1) { // Avançar
      if (status_alvo !== 'aguardando_vgv') {
        ...
        const { data: pf } = await supabase.from('profiles').select('reputation_score').eq('id', corretor_id).single()
        if (pf) {
          await supabase.from('profiles').update({ reputation_score: (pf.reputation_score || 0) + 2 }).eq('id', corretor_id)
        }
        pontos_ganhos = 2
